...
describe('ai-work CLI characterization', () => {
  it('Given a new project, when init runs, then it creates the base project memory layout', async () => {
    await withTempWorkspace(async ({ cwd, env, projectDir }) => {
      ...
      await expect(stat(path.join(projectDir, 'current-focus.md'))).resolves.toBeTruthy();
      await expect(stat(path.join(projectDir, 'QUIRKS.md'))).resolves.toBeTruthy();
  ...
  it('Given initialized AI work storage, when info runs, then it prints valid JSON with configured paths', async () => {
    await withTempWorkspace(async ({ cwd, env, aiWorkHome, projectDir, projectName }) => {
      ...
      expect(info.dirs.progress).toBe(path.join(projectDir, 'progress-updates'));
    });
  });
  ...
  it('Given initialized storage, when context runs, then it prints focus and quirks content', async () => {
    await withTempWorkspace(async ({ cwd, env }) => {
      ...
      expect(result.stdout).toContain('Project-specific patterns, constraints, and reminders for test-project.');
    });
  });
  ...
  it('Given no progress file, when progress path runs, then it creates and prints today’s progress file', async () => {
    await withTempWorkspace(async ({ cwd, env, projectDir }) => {
      ...
      expect(result.exitCode).toBe(0);
      expect(result.stderr).toBe('');
      expect(result.stdout.trim()).toMatch(/progress-updates\/\d{4}-\d{2}-\d{2}-progress\.md$/);
      expect(result.stdout.trim().startsWith(path.join(projectDir, 'progress-updates'))).toBe(true);
  ...
  it('Given a progress source file, when progress append receives a file path, then it appends that file content', async () => {
    await withTempWorkspace(async ({ cwd, env }) => {
      await writeFile(path.join(cwd, 'entry.md'), 'Progress from a file.\n', 'utf8');

      const result = await runCli(['progress', 'append', 'entry.md'], { cwd, env });
  ...
  it('Given no progress content, when progress append runs, then it fails with a helpful error', async () => {
    await withTempWorkspace(async ({ cwd, env }) => {
      const result = await runCli(['progress', 'append'], { cwd, env, input: '' });
  ...
  it('Given a handoff slug, when handoff create and list run, then a pending handoff template exists', async () => {
    await withTempWorkspace(async ({ cwd, env, projectDir }) => {
      const created = await runCli(['handoff', 'create', 'Next Agent'], { cwd, env });

      expect(created.exitCode).toBe(0);
      expect(created.stderr).toBe('');
      expect(created.stdout.trim()).toMatch(/subtask-handoffs\/pending\/\d{4}-\d{2}-\d{2}-\d{2}-\d{2}-\d{2}-next-agent\.md$/);
      ...
      expect(created.stdout.trim().startsWith(path.join(projectDir, 'subtask-handoffs', 'pending'))).toBe(true);
    });
  });

  it('Given a pending handoff, when handoff consume runs, then it prints and moves it to completed', async () => {
    await withTempWorkspace(async ({ cwd, env, projectDir }) => {
      const created = await runCli(['handoff', 'create', 'Follow Up'], { cwd, env });
      ...
      expect(await readdir(path.join(projectDir, 'subtask-handoffs', 'pending'))).toEqual([]);

      const completedDays = await readdir(path.join(projectDir, 'subtask-handoffs', 'completed'));
      expect(completedDays).toHaveLength(1);
      expect(await exists(path.join(projectDir, 'subtask-handoffs', 'completed', completedDays[0], filename))).toBe(true);
    });
  });
  ...
  it('Given a valid doc type and slug, when doc create and list run, then a dated planning doc exists', async () => {
    await withTempWorkspace(async ({ cwd, env, projectDir }) => {
      const created = await runCli(['doc', 'create', 'feature', 'Durable Context'], { cwd, env });

      expect(created.exitCode).toBe(0);
      expect(created.stderr).toBe('');
      expect(created.stdout.trim()).toMatch(/features\/\d{4}-\d{2}-\d{2}-durable-context\.md$/);
      expect(created.stdout.trim().startsWith(path.join(projectDir, 'features'))).toBe(true);
      ...
      const listed = await runCli(['doc', 'list', 'feature', '1'], { cwd, env });
      expect(listed.exitCode).toBe(0);
      expect(listed.stdout.trim()).toBe(path.basename(created.stdout.trim()));
    });
  });

  it('Given an invalid doc type, when doc create runs, then it fails with the current validation message', async () => {
    await withTempWorkspace(async ({ cwd, env }) => {
      const result = await runCli(['doc', 'create', 'unknown', 'Bad Doc'], { cwd, env });

      expect(result.exitCode).toBe(1);
      expect(result.stdout).toBe('');
      expect(result.stderr).toContain('Doc type must be "feature", "architecture", or "research".');
    });
  });
